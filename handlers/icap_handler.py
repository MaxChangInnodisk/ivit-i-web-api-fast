...
from handlers.mesg_handler import handle_exception, http_msg_formatter
from handlers.model_handler import MODEL_URL_DEPLOYER
from utils import gen_uid, get_address

HEAD_MAP = {"json": {"Content-type": "application/json"}}
...
def dict_printer(
    title: str, data: dict, content_maximum: int = 50, level: str = "info"
):
    log_wrapper = {
        "debug": log.debug,
        "info": log.info,
    ...
    for key, val in data.items():
        ...
        _logger(
            "    - {}: {}".format(
                key,
                val if len(val) <= content_maximum else val[:content_maximum] + " ... ",
            )
        )
...
class ICAP_HANDLER:
    def __init__(self, host: str, port: Union[str, int], token: str):
        self.client = mqtt.Client(
            client_id="", clean_session=True, userdata=None, transport="tcp"
        )
        ...
        dict_printer(
            ...
            data={"host": host, "port": port, "token ( pw )": token},
        )
    ...
    def _attr_event(self, data):
        ...
        if "sw_description" in data:
            if "sw_url" in data:
                ...
                log.warning("The second time of iCAP message ...")
        else:
    ...
    def _stream_behaviour(self, exec_data: dict) -> tuple:
        ...
        if action == "run":
            ...
            log.info("Add stream")
            rtc_url, rtc_data = (
                f"http://127.0.0.1:8083/stream/{uid}/add",
                {
                    "name": uid,
                    "channels": {
                        "0": {
    ...
    def _rpc_event(self, request_idx, data):
        ...
        try:
            ...
            if method == "GET":
                ...
                raise KeyError(
                    "Invalid method, sopported is 'GET', 'POST', 'PUT', 'DELETE'"
                )
    ...
    def on_connect(self, client, userdata, flags, rc):
        ...
        if rc == 0:
            ...
            if SERV_CONF["WEB_PORT"] is not None:
            ...
            # }
            basic_attr = {"ivitUrl": host_addr, "ivitTask": get_icap_task_info()}
            self.send_attr(data=basic_attr)

            dict_printer(title="Send basic attributes:", data=basic_attr)

        # Connect Failed
        else:
            log.error("MQTT Got Bad connection. Code:", rc)
...
class ICAP_DEPLOYER(MODEL_URL_DEPLOYER):
    ...
    def __init__(self, data: dict) -> None:
        ...
        self.file_path = os.path.join(
            SERV_CONF["MODEL_DIR"], f"{self.project_name}.zip"
        )
    ...
    def push_mesg(self):
        ...
        if self.is_finish:
            icap_data.update(
                {"current_sw_title": self.title, "current_sw_version": self.ver}
            )
...
def send_post_api(trg_url, data, h_type="json", timeout=5, stderr=True):
    ...
    try:
        resp = requests.post(
            trg_url, data=json.dumps(data), headers=HEAD_MAP[h_type], timeout=timeout
        )
...
def register_tb_device(
    ...
    device_name = (
        device_name if device_type in device_name else f"{device_type}-{device_name}"
    )
    ...
    header = "http://"
    if header not in register_url:
        register_url = header + register_url
    ...
    if not ret:
        dict_printer(
            ...
            data={"URL": register_url, "TOKEN": TB_KEY_TOKEN, "Response": data},
            content_maximum=500,
            level="warn",
        )
        return False, data
    ...
    # Update config
    update_config_for_icap(tb_url=tb_url, tb_port=tb_port, device_name=device_name)

    dict_printer(title="Registering iCAP ... Pass !", data=receive_data)

    return True, data


def init_icap(tb_url: str, tb_port: str, device_name: str) -> tuple:
    """Initialize iCAP"""
    ...
    flag, data = register_tb_device(
        tb_url=tb_url, tb_port=tb_port, device_name=device_name
    )
...
def send_basic_attr():
    try:
        if ("ICAP" in SERV_CONF) and SERV_CONF["ICAP"] is not None:
