...
class DrawTool:
    ...
    def draw_label(
        ...
        (t_wid, t_hei), t_base = cv2.getTextSize(
            label, FONT_TYPE, self.font_size, self.font_thick
        )
    ...
    def draw_area_results(
        self,
        frame: np.ndarray,
        areas: dict,
        color: list = None,
        font_color: list = None,
    ) -> None:
        ...
        for area in areas:
            area_name = area["name"]
            sorted_area_output = sorted(
                area["output"].items(), key=lambda x: x[1], reverse=True
            )
            ...
            for cur_label, cur_nums in sorted_area_output:
                ...
                (t_wid, t_hei), t_base = cv2.getTextSize(
                    result, FONT_TYPE, self.font_size, self.font_thick
                )

                t_xmin = WIDTH_SPACE
                t_ymin = (
                    HEIGHT_SPACE
                    + (HEIGHT_SPACE * cur_idx)
                    + (cur_idx * (t_hei + t_base))
                )
                t_xmax = t_wid + WIDTH_SPACE
                t_ymax = (
                    HEIGHT_SPACE
                    + (HEIGHT_SPACE * cur_idx)
                    + ((cur_idx + 1) * (t_hei + t_base))
                )

                cv2.rectangle(
                    frame, (t_xmin, t_ymin), (t_xmax, t_ymax + t_base), color, -1
                )
                cv2.rectangle(
                    frame, (t_xmin, t_ymin), (t_xmax, t_ymax + t_base), (0, 0, 0), 1
                )
                cv2.putText(
                    frame,
                    result,
    ...
    def get_size(self, text: str) -> tuple:
        (t_wid, t_hei), t_base = cv2.getTextSize(
            text, FONT_TYPE, self.font_size, self.font_thick
        )
